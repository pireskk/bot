...
const BotName = 'piresBot'; 
const instagram = 'tenho n'; 
const aktif = '08:00 - 22:00';
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n' 
            + 'FN:Meliodas\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=https://chat.whatsapp.com/CWDE10hFK31JwBN79IWJLB\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '*'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝙊𝙡𝙖́ 𝙢𝙚𝙪 𝙘𝙖𝙧𝙤 𝙜𝙖𝙛𝙖𝙣𝙝𝙤𝙩𝙤 @${num.split('@')[0]}\n𝙎𝙚𝙟𝙖 𝙗𝙚𝙢 𝙫𝙞𝙣𝙙𝙤 𝙖𝙤 grupo man*  *${mdata.subject}*\n\nespero q goste🐊 ❤️`
				...
				teks = `𝙁𝙡𝙬, espero q um dia volte@${num.split('@')[0]} 🐊🙌😔`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo do Pires. Registre-se para fazer amizade com o bot Pires por meio, \n\nCommand : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar PIRES|17\n\n──「 BOT MAN 」──`,
			...
			const mod = [ownerNumber,"55696103915@s.whatsapp.net","558981246187@s.whatsapp.net"]//ubah nomor lo
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("bot fdp")){
			client.updatePresence(from, Presence.composing)
			reply("vfd seu merda so sabe falar isso")
	}
		...
		if (messagesC.includes("merdakllkk")){
			client.updatePresence(from, Presence.composing)
		...
		if (messagesC.includes("bot")){
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
				...
				if (messagesC.includes("fala oq tu quer")){
			client.updatePresence(from, Presence.composing)
			...
			if (messagesC.includes("fds cara")){
			client.updatePresence(from, Presence.composing)
			...
			break
				case 'ytmp3':
                   ...
                   reply(mess.wait)
					...
					break
				case 'ytmp':
					if (args.length < 1) return reply('Cadê o url, cara?')
